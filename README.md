...
- 2026-05-29 代理配置支持 HTTP/HTTPS 等协议格式；前端增加代理格式实时校验，覆盖登录与编辑入口；同步更新 CLI help 与文档
...
- 多账号 Telegram 管理
- 账号包导入/导出，支持 Telethon 兼容包与 Telegram Desktop TData 包
- 自动签到、定时消息、按钮点击等任务动作
- AI 识图、AI 计算题等自动化动作
...
账号管理页顶部提供“导入账号包”“导出 Telethon”“导出 TData”三个入口，导入和导出的 Zip 包都包含可登录 Telegram 的 session 数据，请按敏感文件保存。

支持的导入格式：

- Telethon 兼容包：每个账号一个目录，包含 `.json` 与 `.session`；兼容 Telegram-Panel 导出的独立 WTelegram session，也兼容 Telethon/Pyrogram SQLite session 和 JSON 内的 `session_string`。
- TData 包：Zip 内包含 `tdata/` 目录，支持单账号包或多账号目录批量导入。
- 可选 `2fa.txt` / `password.txt` 会作为账号二级密码元数据保存，用于后续迁移场景。

导入时默认跳过同名账号；如需替换现有 session，请在导入弹窗勾选“允许覆盖已有账号”。TData 导入和 TData 导出需要 Node 运行时与 `@mtcute/convert`，官方镜像已预装；源码运行时首次使用会自动安装到 `TG_TDATA_RUNTIME_DIR` 指定目录，未设置时使用数据目录下的 `tdata-runtime`。
...
| `TG_TDATA_RUNTIME_DIR` | `/data/tdata-runtime`（示例） | TData 转换依赖安装目录；官方镜像默认使用内置目录，源码运行可按需指定 |
